<div class="footer-contents">
    <div class="menu-footer">
        <div class="menu-col">
            <p>MOVIE</p>
            <ul>
                <li>
                    <a href="#">Now Showing</a>
                </li>
                <li>
                    <a href="#">Coming Soon</a>
                </li>
                <li>
                    <a href="#">Cinemas</a>
                </li>
                <li>
                    <a href="#">News & Activities</a>
                </li>
            </ul>
        </div>
        <div class="menu-col">
            <p>SYSTEM TYPE</p>
            <ul>
                <li>
                    <a href="#">4DX</a>
                </li>
                <li>
                    <a href="#">IMAX</a>
                </li>
                <li>
                    <a href="#">Kids Cinema</a>
                </li>
            </ul>
        </div>
    </div>
    <div class="contact">
        <h5>MOVIE LINE</h5>
        <p>098-888-126</p>
        <div class="social-media">
            <a href="https://www.facebook.com/majorcineplexcambodia/">
                <img src="{{ asset('/assests/')}}//images/facebook-logo.webp" alt="facebook">
            </a>
            <a href="https://www.instagram.com/majorcambodia/">
                <img src="{{ asset('/assests/')}}//images/Instagram.webp" alt="instagram">
            </a>
            <a href="https://page.line.me/majorcineplex">
                <img src="{{ asset('/assests/')}}//images/Line-Logo.jpg" alt="line">
            </a>
            <a href="https://t.me/majorcineplexcambodia">
                <img src="{{ asset('/assests/')}}//images/Telegram_logo.svg.webp" alt="telegram">
            </a>
        </div>
    </div>

</div>

<div class="bottom-footer">
    <div class="app-download">
        <div class="store">
            <img src="./images/play-store.webp" alt="playstore">
        </div>
        <div class="store">
            <img src="./images/app-store.webp" alt="app-store">
        </div>
    </div>
    <div class="copyright">
        <p>Copyright reserved © 2022 MAJOR CINEPLEX GROUP PLC. All rights reserved.</p>
    </div>
</div>
